...
/*****************************************************************************
 * Implementation of public member functions of class LuxExporter.
 *****************************************************************************/

/// Constructs and initialises a new LuxExporter instance.
LuxExporter::LuxExporter(void)
: mReceiver(0),
  mTempParamSet(64)
{}


/// Destroys the LuxExporter instance and frees its resources.
LuxExporter::~LuxExporter(void)
{}
...
Bool LuxExporter::exportScene(BaseDocument& document, LuxAPI& receiver)
{
  // init internal data
  mDocument = &document;
  mReceiver = &receiver;
...
void LuxExporter::Free(void* data)
{
...
void LuxExporter::CopyTo(void* src, void* dst)
{
  if (src && dst) {
...
/*****************************************************************************
 * Implementation of private member functions of class LuxExporter.
 *****************************************************************************/
...
Bool LuxExporter::exportFilm(BaseDocument& document, LuxAPI& receiver)
{
  // obtain render data container
  RenderData* renderData = document.GetActiveRenderData();
  if (!renderData)  ERRLOG_RETURN("LuxExporter::exportFilm(): could not obtain render data");
  BaseContainer* renderSettings = renderData->GetDataInstance();
  if (!renderSettings)  ERRLOG_RETURN("LuxExporter::exportFilm(): could not obtain render settings");
...
/// @return
///   TRUE, if successful, FALSE otherwise
Bool LuxExporter::exportCamera(BaseDocument& document, LuxAPI& receiver)
{
  // check if needed data is available
  if (!mXResolution && !mYResolution) {
    ERRLOG_RETURN("LuxExporter::exportCamera(): frame resolution wasn't calculated yet");
  }

  // obtain camera object and its container
  BaseDraw* view = document.GetRenderBaseDraw();
  if (!view)  ERRLOG_RETURN("LuxExporter::exportCamera(): could not obtain render base draw");
  BaseObject* cameraObj = view->GetSceneCamera(&document);
  if (!cameraObj)  cameraObj = view->GetEditorCamera();
  if (!cameraObj)  ERRLOG_RETURN("LuxExporter::exportCamera(): could not obtain camera");
  if (cameraObj->GetType() != Ocamera)  ERRLOG_RETURN("LuxExporter::exportCamera(): obtained camera object is no camera");
  CameraObject* camera = (CameraObject*)cameraObj;
  BaseContainer* cameraData = camera->GetDataInstance();
  if (!cameraData)  ERRLOG_RETURN("LuxExporter::exportCamera(): couldn't obtain container from camera object");
...
/// @return
///   TRUE, if successful, FALSE otherwise
Bool LuxExporter::exportPixelFilter(BaseDocument& document, LuxAPI& receiver)
{
  mTempParamSet.clear();
...
/// @return
///   TRUE, if successful, FALSE otherwise
Bool LuxExporter::exportSampler(BaseDocument& document, LuxAPI& receiver)
{
  mTempParamSet.clear();
...
/// @return
///   TRUE, if successful, FALSE otherwise
Bool LuxExporter::exportSurfaceIntegrator(BaseDocument& document, LuxAPI& receiver)
{
...
/// @return
///   TRUE, if successful, FALSE otherwise
Bool LuxExporter::exportAccelerator(BaseDocument& document, LuxAPI& receiver)
{
  mTempParamSet.clear();
...
/// @return
///   TRUE, if successful, FALSE otherwise
Bool LuxExporter::exportLights(BaseDocument& document, LuxAPI& receiver)
{
...
/// @return
///   TRUE, if successful, FALSE otherwise
Bool LuxExporter::exportTextures(BaseDocument& document, LuxAPI& receiver)
{
...
/// @return
///   TRUE, if successful, FALSE otherwise
Bool LuxExporter::exportGeometry(BaseDocument& document, LuxAPI& receiver)
{
...
/// @return
///   TRUE, if successful, FALSE otherwise
Bool LuxExporter::exportPolygonObject(PolygonObject& object, LuxAPI& receiver)
{
  // get polygon object
...
/// @return 
///   TRUE, if successful, FALSE otherwise
Bool LuxExporter::convertGeometry(PolygonObject& object,
                                  TrianglesT&    triangles,
                                  PointsT&       points,
                                  NormalsT&      normals)
{
  ...
  if (!triangles.init((mPolygonCache.size() + mQuadCount) * 3)) {
    ERRLOG_RETURN("LuxExporter::convertGeometry(): not enough memory to allocate triangle array");
  }
...
/// @return
///   TRUE if successful, FALSE otherwise.
Bool LuxExporter::convertAndCacheGeometry(PolygonObject& object)
{
  // clear cache
  mPolygonCache.erase();
  mPointCache.erase();
  ...
  if (!mPolygonCache.init(polygonCount)) {
    ERRLOG_RETURN("LuxExporter::convertAndCacheGeometry(): not enough memory to allocate polygon cache");
  }
...
/// @return
///   TRUE if successful, FALSE otherwise.
Bool LuxExporter::convertAndCacheWithoutNormals(PolygonObject& object)
{
  ...
  if (!pointCount || !points || !polygonCount) {
    ERRLOG_RETURN("LuxExporter::convertAndCacheWithoutNormals(): geometry expected, but not found");
  }

  // initialise point map
  FixArray1D<ULONG> pointMap;
  if (!pointMap.init(pointCount)) {
    ERRLOG_RETURN("LuxExporter::convertAndCacheWithoutNormals(): not enough memory to allocate point map");
  }
...
/// @return
///   TRUE if successful, FALSE otherwise.
Bool LuxExporter::convertAndCacheWithNormals(PolygonObject& object,
                                             C4DNormalsT&   normals)
{
  // get points from C4D and polygon count from cache (polygons were already
  // created by (convertAndCacheGeometry())
  ...
  ULONG polygonCount = mPolygonCache.size();
  if (!pointCount || !points || !polygonCount) {
    ERRLOG_RETURN("LuxExporter::convertAndCacheWithNormals(): geometry expected, but not found");
  }

  // initialise point map
  FixArray1D<ULONG> pointMap;
  if (!pointMap.init(pointCount+1)) {
    ERRLOG_RETURN("LuxExporter::convertAndCacheWithNormals(): not enough memory to allocate point map");
  }
  pointMap.fillWithZero();
  ...
  if (point2PolyCount > MAXLONG) {
    ERRLOG_RETURN("LuxExporter::convertAndCacheWithNormals(): point2Poly map will be too big for object '" + object.GetName() + "'");
  }

  // initialise point2poly array
  Point2PolysT point2Polys;
  if (!point2Polys.init(point2PolyCount)) {
    ERRLOG_RETURN("LuxExporter::convertAndCacheWithNormals(): not enough memory to allocate poin2poly map");
  }
  ...
  if (!mPointCache.init(newPointCount) || !mNormalCache.init(newPointCount)) {
    ERRLOG_RETURN("LuxExporter::convertAndCacheWithNormals(): not enough memory to allocate point and/or normal cache");
  }
